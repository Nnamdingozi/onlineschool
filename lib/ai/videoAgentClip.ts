...
(async () => {
  try {
    // Use dynamic import() to load the CommonJS modules.
    const ffmpegStatic = (await import('ffmpeg-static')).default;
    const ffprobeStatic = (await import('ffprobe-static')).default;

    if (ffmpegStatic) {
      ffmpeg.setFfmpegPath(ffmpegStatic);
      console.log('[FFMPEG CONFIG] Portable FFMPEG path set successfully.');
    }
    if (ffprobeStatic) {
      ffmpeg.setFfprobePath(ffprobeStatic.path);
      console.log('[FFMPEG CONFIG] Portable FFPROBE path set successfully.');
    }
  } catch (error) {
    console.error('[FFMPEG CONFIG] ERROR setting portable paths:', error);
  }
})();
